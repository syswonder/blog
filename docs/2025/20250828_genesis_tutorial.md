...
把“连续、平滑、无碰撞”的路径当成一个工程化问题，会更清晰：先定义“什么是好路径”，再选择合适的求解器把它算出来。

#### 1 建模

**空间与变量**

- 在机械臂上，规划最好在**关节空间**做：$q∈R^n$（每个关节一个维度）。路径是连续曲线 $q(t)$。

- 目标可以是终端位姿，也可以是**中间路标**（waypoints）。若是位姿目标，记得先用 IK 得到**多个**可行的关节解作为“目标集合”。

**约束**

- 关节上下限：$q_{min}​≤q(t)≤q_{max}​$

- 速度/加速度/跃度（jerk）限值：∥q˙​∥≤vmax​, ∥q¨​∥≤amax​, ∥q...​​∥≤jmax​

- 无碰撞：对所有时刻，障碍物**有正间隙** d(q(t))≥dsafe​（用最小距离或 signed distance）

**“好路径”的指标（目标函数）**  

常见可加权组合：

- **平滑**：∫∥q¨​(t)∥2dt（最小加加速度）或离散形式 ∑k​∥Δ2qk​∥2

- **短**：∫∥q˙​(t)∥dt 或离散弧长 ∑k​∥qk+1​−qk​∥

- **安全余量**：∑k​ϕ(d(qk​))，ϕ 对小间隙惩罚大（如 CHOMP 的障碍势场）

- **终端精度**：末端与目标位姿的偏差（若允许到达“目标流形”，也可以只罚距离目标集合）

#### 两条主流路线

##### A. 采样‑搜索类

1. 在 q-空间随机采样可行点（碰撞检测过滤）。

2. 建图连边（PRM）或树扩展（RRT），边段用**局部规划器**。

3. 找到从起点到目标集合的无碰撞路径。

4. **后处理**：捷径（shortcutting）、样条拟合（cubic/B‑spline）→ 更短更平滑。

5. **时间参数化**：按速度/加速度/jerk 限制分配时间。
...
1. 先给一条初始轨迹（直线插值、RRT 粗轨迹或多段样条）。

2. 把目标“平滑+短+安全”写进**同一个损失**，碰撞用**距离场**惩罚；

3. 迭代优化（梯度下降、SQP、交替凸化），不断推开障碍、拉直轨迹、抑制抖动。

4. 再做速度曲线生成（同上）。

优点：轨迹天然平滑，能直接考虑动力学/差分约束；  
缺点：局部极值，需好初值和稳定的距离场。

> 实战里常用“**混合**”策略：RRT 找一条安全粗路 → TrajOpt/CHOMP 打磨成丝滑曲线 → TOPP‑RA 时间化。

#### 碰撞检测与距离场

- **广义—精细**两阶段：先用包围盒/栅格做快速“可能相交”（broad‑phase），再用几何库（FCL、GJK+EPA、签距场 SDF）做精细距离。

- 用**签距场（SDF）**可高效提供：距离 d、法向 ∇d，优化时可直接用作梯度。

- 连续碰撞：边段上做**稠密采样**或用**连续碰撞检测（CCD）**，避免“穿隧”。

#### 时间参数化

- 在已定的几何路径 q(s) 上求 s(t)（弧长到时间），满足速度/加速度/jerk 上限与关节耦合（通过链式法则）。

- 工程做法：
  
  - **梯形速度**或**S 曲线**（jerk 限制，启动/停止更柔和）
  
  - **TOPP‑RA** 等最优时间标定算法，自动在每段上推导最大可行速度曲线并前后向传播。
